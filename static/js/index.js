...
const doInsertColors = function (level) {
  ...
  if (
    ...
    (level >= 0 && colors[level] === undefined)
  ) {
    return;
  }
...
const aceEditEvent = (hook, call) => {
  ...
  setTimeout(() => {
    ...
    const startAttribs = attrManager.getAttributesOnPosition(
      rep.selStart[0],
      rep.selStart[1]
    );
    const endAttribs = attrManager.getAttributesOnPosition(
      rep.selEnd[0],
      rep.selEnd[1]
    );
